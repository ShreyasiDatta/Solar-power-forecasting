...
# MODIFIED: Changed results and tuner directories for the new 'wind' project
RESULTS_DIR = "project_results_wind_v1"
DATA_FILE = 'time_series_60min_singleindex.csv'
TUNER_DIR = "kerastuner_logs_wind"
...
#################################################################
### 2. Load Real Data (MODIFIED for Wind Generation)
#################################################################
print(f"Step 2: Loading and preprocessing data from {DATA_FILE}...")

# MODIFIED: Changed 'DE_solar_generation_actual' to 'DE_wind_generation_actual'
feature_cols_to_load = [
    'utc_timestamp',
    'DE_wind_generation_actual',  # <-- This is the only column name that changed
...
try:
    ...
    print(f"Please check if '{feature_cols_to_load[1]}' and '{feature_cols_to_load[2]}' exist in your file.")
    print(f"Full error: {e}")
    sys.exit()

# MODIFIED: Changed the rename key to match the new wind column
df.rename(columns={
    'DE_wind_generation_actual': 'power', # <-- This was changed
    'DE_load_actual_entsoe_transparency': 'load'
}, inplace=True)

# --- The rest of the script is identical from here ---
# It uses the generic 'power' column, which we just created.
...
TARGET_COL_INDEX = 0  # 'power' is still the first column (index 0)
N_FEATURES = len(feature_columns)

print(f"Loaded {len(df)} records. Using {N_FEATURES} features (Target: Wind Power).")
